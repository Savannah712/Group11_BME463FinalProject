// bme463_sender_final_lab_project
...
const int DP_SIZE = 6;   // Number of bytes in a single data point (4 data points and newline character)
// DP_SIZE = 10 for class signals and 6 for our signal
...
void send_sp(void) {
  ...
  for (int i = 0; i < DP_SIZE; i++)
      tbuff[i] = readc();
   // Replace '\n' with '\0';
  tbuff[DP_SIZE - 1] = '\0'; // windows code takes up 2 bits for newline character (CRLF)
   // Convert string to short
  temp.s = (short) atoi(&tbuff[0]); // start pulling data at first index
   // Send data
